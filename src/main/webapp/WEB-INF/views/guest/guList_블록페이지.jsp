	...
	<div class="container">
  	...
  	<c:forEach var="vo" items="${vos}" varStatus="st" >
		...
		<div class="text-center">
			...
			<c:if test="${curBlock > 0}">[<a href="${ctp}/guList.gu?pag=${(curBlock-1)*blockSize + 3}">이전페이지</a>]</c:if>
			<c:forEach var="i" begin="${(curBlock*blockSize) + 1}" end="${(curBlock*blockSize) +blockSize}" varStatus="st">
				<c:if test="${i <= totPage}">
					[<a href="${ctp}/guList.gu?pag=${i}">${i}</a>]
				</c:if>
			</c:forEach> 
			<c:if test="${curBlock < lastBlock}">[<a href="${ctp}/guList.gu?pag=${(curBlock+1)*blockSize + 1}">다음페이지</a>]</c:if>
